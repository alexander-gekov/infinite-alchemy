...
For **Black Forest FLUX** image models, requests default to **`aspect_ratio` `1:1`** (OpenRouter’s documented preset, ~1 MP). To try smaller outputs via explicit `width`/`height` in `image_config`, set **both** `OPENROUTER_FLUX_IMAGE_WIDTH` and `OPENROUTER_FLUX_IMAGE_HEIGHT` (minimum 64 each); this path depends on provider support. If you pass `aspect_ratio` from the AI image API, those presets apply instead. Gemini’s smaller `image_size` values (e.g. `0.5K`) apply only to the models documented for that on OpenRouter.
